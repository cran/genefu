...
function(data, annot, do.mapping=FALSE, mapping, do.scale=TRUE) {
	...
	sbt.id <- subtype.cluster.predict(sbt.model=sbtmod1, data=data, annot=annot, do.mapping=do.mapping, mapping=mapping, do.scale=do.scale, do.prediction.strength=FALSE, do.BIC=FALSE, plot=FALSE, verbose=FALSE)

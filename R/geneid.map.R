...
function(geneid1, data1, geneid2, data2, verbose=FALSE) {
	...
	if(length(gid.dupl) > 0) {
		if(verbose) { cat("\ndataset1 duplicates...\n") }
		mysd <- cbind("probe"=gg.dupl, "gid"=geneid1[gg.dupl], "sd"=apply(X=data1[ , gg.dupl, drop=FALSE], MARGIN=2, FUN=sd, na.rm=TRUE))
		mysd <- mysd[order(mysd[ , "sd"], decreasing=TRUE, na.last=TRUE), , drop=FALSE]
	...
	if(is.null(data2)) {
		...
		if(length(gid.dupl) > 0) {
			if(verbose) { cat("\ndataset2 duplicates...\n") }
			mysd <- cbind("probe"=gg.dupl, "gid"=geneid2[gg.dupl], "sd"=apply(X=data2[ , gg.dupl, drop=FALSE], MARGIN=2, FUN=sd, na.rm=TRUE))
			mysd <- mysd[order(mysd[ , "sd"], decreasing=TRUE, na.last=TRUE), , drop=FALSE]
			mysd <- mysd[!duplicated(mysd[ , "gid"]), , drop=FALSE]
